...
async function loadRemoteConfig(tokenConfig = null, repo = null) {
  try {
    ...
    if (response.ok) {
      ...
      saveConfig(STORE_CONFIG); // حفظ محلياً
      ...
      console.log('⚠️ لم يتم العثور على الإعدادات على GitHub، استخدام الإعدادات المحلية');
    }
  } catch (error) {
...
function loadConfig() {
  ...
  if (saved) {
    try {
      ...
      // دمج الإعدادات مع الحفاظ على الهيكل الأساسي
      STORE_CONFIG = { 
        ...STORE_CONFIG,
        ...parsedConfig,
        STORE_INFO: { ...STORE_CONFIG.STORE_INFO, ...parsedConfig.STORE_INFO },
        DISCOUNTS: { ...STORE_CONFIG.DISCOUNTS, ...parsedConfig.DISCOUNTS }
      };
